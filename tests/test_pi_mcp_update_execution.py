...
from runtasks.database import _create_decision_execution_outcomes
...
class PiMcpUpdateExecutionCliTests(unittest.TestCase):
    ...
    def downgrade_to_schema_eight(self, database_path: Path) -> None:
        with sqlite3.connect(database_path) as connection:
            outcomes = connection.execute(
                """
                SELECT decision_id, approval_run_id, status, summary,
                       details_json, completed_at, notification_status,
                       notification_attempts, notification_claimed_at,
                       notification_last_attempt_at, notification_last_error,
    ...
    def test_schema_eight_migration_preserves_history_and_refuses_inflight_work(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            database_path = home / "var" / "data" / "runtasks.sqlite3"
            self.downgrade_to_schema_eight(database_path)
